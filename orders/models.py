...
class Cart(models.Model):
    ...
    def __str__(self):
        return f"Корзина пользователя: {self.user.username} (№{self.cart_number})"
...
class Order(models.Model):
    ...
    def clean(self):
        if not self.cart_number:
            raise ValidationError("Номер корзины обязателен для заказа.")
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.pk:  # Только при создании
            try:
                ...
                raise ValidationError("Корзина с данным номером не найдена.")
    ...
    def __str__(self):
        return f"Заказ {self.cart_number} пользователя {self.user.username}"
...
class OrderItem(models.Model):
    ...
    def __str__(self):
        return f"{self.product.name} в заказе {self.order.cart_number}"
